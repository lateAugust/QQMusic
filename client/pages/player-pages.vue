...
export default {
  ...
  render() {
    ...
    return (
      <section class="player-pages">
        ...
        <div class={{ bg: true, 'd-none': this.showLyric }}>
          <img
            src={this.player.album_big}
            width="100%"
            height="100%"
        ...
        <div class={['lyrics', { 'd-none': !this.showLyric }, 'position-relative']}>
          <ul class="position-absolute">
            {this.lyricArr.map((item, i) => (
              <li class={{ 'text-center': true, active: this.lyricIndex === i }}>
                {item.text || '-----------------------'}
              </li>
            ))}
          </ul>
        ...
        <ul class="bar row  align-items-center justify-content-around">
          ...
          <li class="col-7 progress" onClick={this.changeBar.bind(this, true, 0)} ref="playProgress">
            <div
              ...
              role="progressbar"
              style={{ width: tooFn.barWidth(this.playBarWidth, this.playing / this.durationTime) + 'px' }}
              aria-valuenow="0"
              aria-valuemin="0"
              aria-valuemax="100"
              onClick={this.changeBar.bind(this, false, 0)}
            ></div>
          </li>
        ...
        <div class="lyric row align-items-center">
          ...
          <div class="col-8 progress" onClick={this.changeBar.bind(this, true, 1)}>
            <div
              ...
              aria-valuemin="0"
              aria-valuemax="100"
              onClick={this.changeBar.bind(this, false, 1)}
            ></div>
          </div>
        </div>
      </section>
  ...
  methods: {
    ...
    requestSongSrc() {
      // 获取src
      tooFn
        .requestSong(`/music?songid=226989116&songmid=${this.player.songmid}`)
        .then(({ data }) => {
          if (data.isPath) {
            this.src = data.path;
          }
    ...
    requestLyric() {
      tooFn
        .requestSong(`/lyric?lyrics=${this.player.songmid}`)
        .then(({ data }) => {
