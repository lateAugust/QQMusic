...
export default {
          ...
          <ul class="list" style={{ height: this.wrapHeight / 2 - 75 + 'px' }}>
            {this.songListArr.map((item, i) => (
              <li class="row justify-content-around align-items-center">
                <div
                  class={['serial', 'col-1', 'text-center', { active: i === this.index }]}
                  onClick={this.playerSong.bind(this, i, item)}
                >
                  {tooFn.addZero(i)}
                </div>
                <div class="col-8 text-left ">
                  <span
                    class={['song_header', 'd-inline-block', { active: i === this.index }]}
                    onClick={this.playerSong.bind(this, i, item)}
                  >
                    {item.songname}
                  </span>
                  <span
                    class={['singer', 'd-inline-block', { active: i === this.index }]}
                    onClick={this.playerSong.bind(this, i, item)}
                  >
                    -{tooFn.codeSinger(item)}
                  </span>
                </div>
  ...
  methods: {
    ...mapMutations(['changeSongPlayerPages', 'assignmentPlayer']),
    songPlayerPages() {
    ...
    playerSong(i, item) {
      ...
      tooFn
        .requestSong(`/music?songid=226989116&songmid=${item.songmid}`)
        .then(({ data }) => {
          if (data.isPath) {
            this.src = data.path;
          }
