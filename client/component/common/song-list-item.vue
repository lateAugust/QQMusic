...
export default {
      ...
      <div class="box">
        <header>
          ...
          {this.listOrSearch ? (
            ...
            <label-list routerType={this.params} onChange={this.changeSongListId}></label-list>
          )}
        </header>
        <ul class={[{ 'd-none': this.loaded }, 'row']}>
          {arr.map((item, i) => (
            <li class="col-12-39" onClick={this.enterPlayerPages.bind(this, item, i)}>
  ...
  methods: {
    ...
    requestSearch(p = 1) {
      ...
      tooFn
        .requestSong(`/query?s=${this.searchQuery}&page=${p}&num=30`)
        .then(({ data, curnum, curpage, totalnum }) => {
          console.log(data);
    ...
    requestTop() {
      tooFn
        .requestSong(`/top?topidurl=${this.songListId}`)
        .then(({ data }) => {
